...
class SE(nn.Module):
    ...
    Steps:
      1) Squeeze: global average pooling to (B, C, 1, 1)
      2) Excitation: 2-layer channel MLP (implemented as 1x1 conv) to (B, C, 1, 1)
      3) Scale: channel-wise multiplication (broadcast over H, W)
    """
    def __init__(self, channels: int, reduction: int = 16):
        """
        Args:
            channels: input channel count C
            reduction: reduction ratio r, hidden = C // r (commonly 16)
        """
        super().__init__()
        assert channels > 0
        hidden = max(1, channels // reduction)  # avoid 0 when C is small

        # 1) Squeeze: (B,C,H,W) -> (B,C,1,1)
        self.gap = nn.AdaptiveAvgPool2d(1)

        # 2) Excitation: 2-layer channel MLP via 1x1 conv
        #    (B,C,1,1) -> (B,hidden,1,1) -> (B,C,1,1)
        self.fc1 = nn.Conv2d(channels, hidden, kernel_size=1, bias=True)
        self.act = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv2d(hidden, channels, kernel_size=1, bias=True)

        # 3) constrain weights to [0, 1]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x: (B, C, H, W)
        """
        _b, _c, _h, _w = x.shape  # for shape readability only
        ...
        # ---- Excitation ----
        # s: (B, C, 1, 1), one weight per channel in [0, 1]
        s = self.fc2(self.act(self.fc1(z)))
        s = self.sigmoid(s)

        # ---- Scale (broadcast multiply) ----
        # x * s: s broadcasts over H,W so output stays (B, C, H, W)
        out = x * s
...
class CBAM(nn.Module):
    ...
    Two sequential sub-modules:
      1) Channel attention: uses GAP/GMP and a shared MLP to generate channel weights
      2) Spatial attention: uses channel-wise avg/max pooling and a conv to generate spatial weights
    """
    ...
    def __init__(self, channels: int, reduction: int = 16, spatial_kernel: int = 7):
        ...
        Args:
            channels: input channel count C
            reduction: reduction ratio r for channel attention (commonly 16)
            spatial_kernel: spatial attention kernel size (commonly 7), must be odd
        """
        super().__init__()
        assert channels > 0
        ...
        if spatial_kernel % 2 == 0:
            raise ValueError(f"CBAM spatial_kernel must be odd, got {spatial_kernel}")
        self.spatial_conv = nn.Conv2d(2, 1, kernel_size=spatial_kernel, padding=spatial_kernel // 2, bias=False)
        self.sigmoid_s = nn.Sigmoid()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # ---- Channel Attention ----
        # (B,C,1,1)
        # Use reduction-based global max pooling to avoid AdaptiveMaxPool2d deterministic CUDA backward limitation.
        max_pool = torch.amax(x, dim=(2, 3), keepdim=True)
        ca = self.mlp(self.avg_pool(x)) + self.mlp(max_pool)
        ca = self.sigmoid_c(ca)
        x = x * ca

        # ---- Spatial Attention ----
        # (B,1,H,W) from channel-wise avg/max
        avg_out = torch.mean(x, dim=1, keepdim=True)
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        sa = self.spatial_conv(torch.cat([avg_out, max_out], dim=1))
        sa = self.sigmoid_s(sa)
        x = x * sa
